...
const API_BASE = '/api';
const MOCK_AUTH = true; // Enable client-side auth for demo
...
function printAttendanceReport() {
  ...
  filtered.forEach((rec) => {
    const emp = state.employees.find((e) => e.id === rec.employeeId);
    const statusColor = rec.status === 'present' ? '#4CAF50' : rec.status === 'absent' ? '#f44336' : '#ff9800';
    tableRows += `
      <tr>
...
async function loadProtectedData() {
  if (MOCK_AUTH) {
    ...
    if (window.EMBEDDED_DATA) {
      Object.assign(state, {
        user: { username: 'admin', fullName: 'Lungile Moyo', role: 'HR Manager' },
  ...
  // Original API calls
  const [me, employees, timeOffRequests, attendanceRecords, payrollRecords] = await Promise.all([
    api('/auth/me'),
    api('/employees'),
    api('/timeoff'),
    api('/attendance'),
...
async function renderLogin() {
  ...
  document.getElementById('login-terminal-icon').innerHTML = icon('terminal', 12);
...
async function handleLogin(event) {
  ...
  try {
    ...
    if (MOCK_AUTH) {
      ...
      state.user = { username: 'admin', fullName: 'Lungile Moyo', role: 'HR Manager' };
...
function attachEmployeesEvents() {
  ...
  document.querySelectorAll('[data-delete-employee]').forEach((button) => {
    button.addEventListener('click', async () => {
      ...
      if (!confirm('Delete this employee? This also removes their attendance, payroll and time-off records.')) return;
      try {
        await api(`/employees/${id}`, { method: 'DELETE' });
        state.employees = state.employees.filter((emp) => emp.id !== id);
        state.payrollRecords = state.payrollRecords.filter((r) => r.employeeId !== id);
        state.attendanceRecords = state.attendanceRecords.filter((r) => r.employeeId !== id);
        state.timeOffRequests = state.timeOffRequests.filter((r) => r.employeeId !== id);
...
function openEmployeeModal(id = null) {
  ...
  document.getElementById('employee-form').addEventListener('submit', async (event) => {
    ...
    try {
      if (employee) {
        const updated = await api(`/employees/${employee.id}`, { method: 'PUT', body: JSON.stringify(values) });
        state.employees = state.employees.map((emp) => emp.id === employee.id ? updated : emp);
        toast('Employee updated!');
      } else {
        const created = await api('/employees', { method: 'POST', body: JSON.stringify(values) });
...
function attachPayrollEvents() {
  ...
  document.querySelectorAll('[data-process-payroll]').forEach((button) => button.addEventListener('click', async () => {
    const id = button.getAttribute('data-process-payroll');
    try {
      const updated = await api(`/payroll/${id}/process`, { method: 'PUT' });
      state.payrollRecords = state.payrollRecords.map((r) => r.id === id ? updated : r);
...
function openPayslip(id) {
  ...
  `);
  document.getElementById('download-current-payslip')?.addEventListener('click', () => printPayslip(id));
}
...
function attachTimeOffEvents() {
  ...
  document.querySelectorAll('[data-approve-request]').forEach((button) => button.addEventListener('click', async () => {
    ...
    try {
      ...
      state.timeOffRequests = state.timeOffRequests.map((r) => r.id === id ? updated : r);
...
function openLeaveModal() {
  ...
  document.getElementById('leave-form').addEventListener('submit', async (event) => {
    ...
    try {
      const created = await api('/timeoff', { method: 'POST', body: JSON.stringify(values) });
...
function attachAttendanceEvents() {
  document.getElementById('export-attendance')?.addEventListener('click', () => printAttendanceReport());
