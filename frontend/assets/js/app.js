...
// Template cache for loading HTML templates
const templates = {};
...
// Central fetch wrapper: attaches the JWT (if we have one), always sends/
// expects JSON, and turns non-2xx responses into thrown Errors carrying the
...
function icon(name, size = 16, extra = "") {
  ...
  const paths = {
    ...
    dollar:
      '<line x1="12" x2="12" y1="2" y2="22"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7H14a3.5 3.5 0 0 1 0 7H6"/>',
    calendar:
      '<rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" x2="16" y1="2" y2="6"/><line x1="8" x2="8" y1="2" y2="6"/><line x1="3" x2="21" y1="10" y2="10"/>',
    clipboard:
      '<rect x="8" y="2" width="8" height="4" rx="1"/><path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2"/><path d="M8 12h8M8 16h5"/>',
    logout:
...
function formatMonth(month) {
  return dateKey(`${month}-01`).toLocaleDateString("en-ZA", {
    month: "short",
    timeZone: "UTC",
  });
}
...
function monthlyAttendanceTrend() {
  ...
  state.attendanceRecords.forEach((record) => {
    ...
    const entry = months.get(month) || { present: 0, absent: 0, total: 0 };
    entry.total += 1;
  ...
  return [...months.entries()]
    ...
    .map(([month, entry]) => ({
      ...
      present: Math.round((entry.present / entry.total) * 100),
      absent: Math.round((entry.absent / entry.total) * 100),
    }));
}

function weeklyAttendanceTrend(selectedDate) {
  ...
  return ["Mon", "Tue", "Wed", "Thu", "Fri"].map((day, index) => {
    ...
    const records = state.attendanceRecords.filter((record) => record.date === date);
    return {
      day,
...
// Print functions for payslips and attendance reports
function printPayslip(id) {
  const rec = state.payrollRecords.find((r) => r.id === id);
  if (!rec) {
    toast("Payslip not found", "error");
    return;
  }
  ...
  const printContent = `
    ...
    <html>
    <head>
      <title>Payslip - ${rec.employeeName}</title>
      <style>
        ...
        .total-row { border-top: 2px solid #333; border-bottom: 2px solid #333; font-weight: bold; padding: 12px 0 !important; }
        .net-row { background-color: #f0f0f0; font-size: 16px; padding: 12px 0 !important; }
        .footer { text-align: center; font-size: 11px; color: #666; margin-top: 30px; border-top: 1px solid #ddd; padding-top: 15px; }
        @media print {
          body { margin: 0; }
    ...
    <body>
      <div class="header">
        <div class="company-name">MODERN TECH SOLUTIONS</div>
        <div class="payslip-title">EMPLOYEE PAYSLIP</div>
        <div>${rec.month.toUpperCase()} ${rec.year}</div>
      </div>
      
      <div class="employee-info">
        <div class="info-section">
          <div class="info-label">Employee Name</div>
          <div class="info-value">${escapeHtml(rec.employeeName)}</div>
          <div class="info-label">Employee ID</div>
          <div class="info-value">${escapeHtml(rec.employeeId)}</div>
        </div>
        <div class="info-section">
          <div class="info-label">Department</div>
          <div class="info-value">${escapeHtml(emp?.department || "N/A")}</div>
          <div class="info-label">Position</div>
          <div class="info-value">${escapeHtml(emp?.position || "N/A")}</div>
        </div>
      </div>
      ...
      <div class="section">
        ...
        <table>
          <tr>
            ...
            <td class="amount">R ${Number(rec.baseSalary).toFixed(2)}</td>
          </tr>
          <tr>
            <td class="label">Bonus</td>
            <td class="amount">R ${Number(rec.bonus).toFixed(2)}</td>
          </tr>
          <tr>
            <td class="label">Overtime (${rec.overtimeHours} hours)</td>
            <td class="amount">R ${Number(overtimePay).toFixed(2)}</td>
          </tr>
          <tr class="total-row">
            <td class="label">GROSS EARNINGS</td>
            <td class="amount">R ${Number(grossEarnings).toFixed(2)}</td>
          </tr>
        </table>
      </div>
      ...
      <div class="section">
        ...
        <table>
          <tr>
            ...
            <td class="amount">R ${Number(paye).toFixed(2)}</td>
          </tr>
          <tr>
            <td class="label">UIF</td>
            <td class="amount">R ${Number(uif).toFixed(2)}</td>
          </tr>
          <tr>
            <td class="label">Medical Aid</td>
            <td class="amount">R ${Number(medical).toFixed(2)}</td>
          </tr>
          <tr>
            <td class="label">Retirement Fund</td>
            <td class="amount">R ${Number(retirement).toFixed(2)}</td>
          </tr>
          <tr class="total-row">
            <td class="label">TOTAL DEDUCTIONS</td>
            <td class="amount">R ${Number(rec.deductions).toFixed(2)}</td>
          </tr>
        </table>
      </div>
      ...
      <table>
        <tr class="net-row">
          <td class="label" style="font-size: 16px;">NET SALARY</td>
          <td class="amount" style="font-size: 18px;">R ${Number(rec.netSalary).toFixed(2)}</td>
        </tr>
      </table>
      
      <div class="footer">
        <p>Computer-generated payslip · Generated on ${new Date().toLocaleDateString("en-ZA")}</p>
        <p>For queries, please contact HR Department</p>
      </div>
      
      <div class="no-print" style="margin-top: 20px; text-align: center;">
        <button onclick="window.print(); return false;" style="padding: 10px 20px; font-size: 14px; cursor: pointer;">Print</button>
        <button onclick="window.close(); return false;" style="padding: 10px 20px; font-size: 14px; cursor: pointer; margin-left: 10px;">Close</button>
      </div>
    </body>
    </html>
  `;
...
function printAttendanceReport() {
  ...
  if (filtered.length === 0) {
    toast("No records to print for this date", "error");
    return;
  }
  ...
  filtered.forEach((rec) => {
    ...
    tableRows += `
      <tr>
        ...
        <td>${escapeHtml(rec.checkIn)}</td>
        <td>${escapeHtml(rec.checkOut)}</td>
        <td>${rec.hoursWorked > 0 ? rec.hoursWorked + "h" : "—"}</td>
        <td style="text-align: center; color: ${statusColor}; font-weight: bold;">${rec.status.toUpperCase()}</td>
      </tr>
    `;
  });
  ...
  const printContent = `
    ...
    <html>
    <head>
      <title>Attendance Report - ${ui.attendanceDate}</title>
      <style>
        body { font-family: Arial, sans-serif; margin: 20px; color: #333; }
        .header { text-align: center; border-bottom: 2px solid #333; padding-bottom: 20px; margin-bottom: 30px; }
        .company-name { font-size: 24px; font-weight: bold; margin-bottom: 5px; }
        .report-title { font-size: 18px; margin: 10px 0; }
        .summary { display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; gap: 15px; margin-bottom: 30px; background-color: #f5f5f5; padding: 15px; border-radius: 5px; }
        .summary-item { text-align: center; }
        .summary-label { font-weight: bold; color: #666; font-size: 12px; text-transform: uppercase; }
        .summary-value { font-size: 20px; font-weight: bold; color: #333; margin-top: 5px; }
        table { width: 100%; border-collapse: collapse; }
        th { background-color: #333; color: white; padding: 12px; text-align: left; font-weight: bold; }
    ...
    <body>
      <div class="header">
        ...
        <div>${new Date(ui.attendanceDate + "T00:00:00").toLocaleDateString("en-ZA", { dateStyle: "full" })}</div>
      </div>
      ...
      <div class="summary">
        ...
        <div class="summary-item">
          <div class="summary-label">Avg Hours Worked</div>
      ...
      <div class="footer">
        <p>Attendance Report Generated on ${new Date().toLocaleDateString("en-ZA")} at ${new Date().toLocaleTimeString()}</p>
...
// Called once we have a valid JWT (right after login, or on page load if a
// token is already sitting in localStorage from a previous session).
async function loadProtectedData() {
  if (MOCK_AUTH) {
    // Load embedded data from embeddedData.js
    if (window.EMBEDDED_DATA) {
      Object.assign(state, {
        ...
        employees: window.EMBEDDED_DATA.employees || [],
        timeOffRequests: window.EMBEDDED_DATA.timeOffRequests || [],
        attendanceRecords: window.EMBEDDED_DATA.attendanceRecords || [],
        payrollRecords: window.EMBEDDED_DATA.payrollRecords || [],
      });
      return;
    }
  }
  ...
  // Original API calls
  const [me, employees, timeOffRequests, attendanceRecords, payrollRecords] =
    await Promise.all([
      api("/auth/me"),
      api("/employees"),
      api("/timeoff"),
...
async function renderLogin() {
  ...
  // Fill in icons
  document.querySelector(".logo-icon-lg").innerHTML = icon("zap", 24);
  document.getElementById("login-terminal-icon").innerHTML = icon(
    "terminal",
    12,
  );
  document.getElementById("login-user-icon").innerHTML = icon("user", 10);
  document.getElementById("login-lock-icon").innerHTML = icon("lock", 10);
...
async function handleLogin(event) {
  ...
  if (!username) {
    setLoginError("username", "Username required");
    valid = false;
  }
  if (!password) {
    setLoginError("password", "Password required");
    valid = false;
  }
  if (!valid) return;
  ...
  try {
    ...
    setLoginError("username", error.message || "Invalid credentials");
    document.getElementById("login-card")?.classList.add("shake");
    setTimeout(
      () => document.getElementById("login-card")?.classList.remove("shake"),
      650,
    );
    toast(`ACCESS DENIED — ${error.message || "Invalid credentials"}`, "error");
  }
}
    ...
    <div class="app-shell">
      ...
      <div class="main-wrap">
        <header class="topbar">
          ...
          <div class="profile">
            <button id="theme-toggle" class="icon-btn" aria-label="Toggle theme" title="Toggle light/dark mode">${icon(themeManager?.isDark() ? "sun" : "moon", 16)}</button>
  ...
  return `
    <div class="sidebar-inner">
      <div class="brand-row">
        ...
        <div>
          <h1 class="brand-title">MODERN_TECH<br/>SOLUTIONS</h1>
...
function startClock() {
  const update = () => {
    const value = new Date().toLocaleTimeString("en-ZA", {
      hour: "2-digit",
      minute: "2-digit",
      second: "2-digit",
    });
    document.querySelectorAll(".brand-time").forEach((el) => {
...
function attachShellEvents() {
  ...
  document
    ...
    ?.addEventListener("click", closeMobileMenu);
  document
    .querySelectorAll(".mobile-drawer .nav-link")
  ...
  // Listen for theme changes and update button icon
  window.addEventListener("themechange", (e) => {
    const btn = document.getElementById("theme-toggle");
    if (btn) {
      const isDark = e.detail.theme === "dark";
      btn.innerHTML = icon(isDark ? "sun" : "moon", 16);
    }
  });
}
...
function statCard({
  ...
  const shown =
    display ?? `${prefix}<span data-count-to="${value}">0</span>${suffix}`;
  return `
...
function renderDashboard() {
  ...
  const onLeaveEmployees = state.employees.filter(
    (e) => e.status === "on-leave",
  ).length;
  const pendingTimeOff = state.timeOffRequests.filter(
    (r) => r.status === "pending",
  ).length;
  ...
  const stats = [
    {
      ...
      sub: `${activeEmployees} active · ${onLeaveEmployees} on leave`,
      iconName: "users",
      color: NEON_GREEN,
      delay: 0,
    },
    ...
    },
    {
      ...
      sub: "Time off requests awaiting",
      iconName: "clock",
      color: NEON_PURPLE,
      delay: 160,
    },
    {
      label: "Total Payroll",
      value: Math.round(totalPayroll / 1000),
      sub: `${state.payrollRecords.length} employees processed`,
      iconName: "dollar",
      color: NEON_GOLD,
      prefix: "R",
      suffix: "K",
...
function renderEmployees() {
  ...
  const filtered = state.employees.filter((emp) => {
    ...
    return (
      matches &&
      (ui.employeeDepartment === "all" ||
        emp.department === ui.employeeDepartment)
    );
  });
          ...
          <div class="table-wrap">
            <table>
              ...
              <tbody>
                ${
                  filtered.length === 0
                    ? `<tr><td colspan="7" class="no-records">// NO RECORDS FOUND</td></tr>`
                    : filtered
                        .map(
                          (emp) => `
                  <tr>
...
function attachEmployeesEvents() {
  ...
  document.querySelectorAll("[data-edit-employee]").forEach((button) => {
    button.addEventListener("click", () =>
      openEmployeeModal(button.getAttribute("data-edit-employee")),
    );
  });
  document.querySelectorAll("[data-delete-employee]").forEach((button) => {
    button.addEventListener("click", async () => {
      ...
      if (
        !confirm(
          "Delete this employee? This also removes their attendance, payroll and time-off records.",
        )
      )
        return;
      try {
        ...
        toast(error.message || "Could not delete employee", "error");
      }
    });
  });
}
  ...
  document
    ...
    .addEventListener("submit", async (event) => {
      ...
      try {
        ...
        // Server-side field errors (e.g. duplicate email) map onto the same inputs.
        if (error.fields) showFormErrors(error.fields);
        toast(error.message || "Could not save employee", "error");
      }
    });
}
...
function renderPayroll() {
  ...
  const stats = [
    ...
    {
      label: "GROSS",
      display: formatMoney(totalGross),
      iconName: "trendDown",
      color: NEON_GREEN,
    },
    {
      label: "DEDUCTIONS",
      display: formatMoney(totalDeductions),
      iconName: "trendDown",
      color: NEON_RED,
    },
    {
      label: "NET PAYOUT",
      display: formatMoney(totalNet),
      iconName: "dollar",
      color: NEON_PURPLE,
    },
  ];
          ...
          <div class="table-wrap">
            <table>
              ...
              <tbody>
                ${
                  filtered.length === 0
                    ? `<tr><td colspan="9" class="no-records">// NO RECORDS FOR THIS PERIOD</td></tr>`
                    : filtered
                        .map(
                          (rec) => `
                  <tr>
...
function attachPayrollEvents() {
  ...
  document.getElementById("payroll-year")?.addEventListener("change", (e) => {
    ui.payrollYear = e.target.value;
    render();
  });
  document
    .querySelectorAll("[data-view-payslip]")
    .forEach((button) =>
      button.addEventListener("click", () =>
        openPayslip(button.getAttribute("data-view-payslip")),
      ),
    );
...
function openPayslip(id) {
  ...
  const medical = Number((rec.deductions * 0.15).toFixed(2));
  const retirement = Number((rec.deductions * 0.05).toFixed(2));
  const row = (label, value, color = "#f0f4ff") =>
    `<div class="pay-row"><span>${label}</span><span style="color:${color}">${value}</span></div>`;
...
function renderTimeOff() {
  ...
  const pending = state.timeOffRequests.filter(
    (r) => r.status === "pending",
  ).length;
  const approved = state.timeOffRequests.filter(
    (r) => r.status === "approved",
  ).length;
  const rejected = state.timeOffRequests.filter(
    (r) => r.status === "rejected",
  ).length;
  const stats = [
          ...
          <div class="table-wrap">
            <table>
              ...
              <tbody>
                ${
                  filtered.length === 0
                    ? `<tr><td colspan="8" class="no-records">// NO REQUESTS FOUND</td></tr>`
                    : filtered
                        .map(
                          (req) => `
                  <tr>
...
function attachTimeOffEvents() {
  ...
  document.querySelectorAll("[data-timeoff-filter]").forEach((button) =>
    button.addEventListener("click", () => {
  ...
  document.querySelectorAll("[data-approve-request]").forEach((button) =>
    button.addEventListener("click", async () => {
      ...
      try {
        ...
        toast(error.message || "Could not approve request", "error");
      }
    }),
  );
...
function openLeaveModal() {
    ...
    <form id="leave-form">
      ...
      <div class="form-row" style="margin-bottom:16px">
        ...
        <select id="leave-type" name="type" class="select">
          <option value="annual">Annual Leave</option><option value="sick">Sick Leave</option><option value="personal">Personal</option><option value="unpaid">Unpaid Leave</option><option value="maternity">Maternity Leave</option><option value="paternity">Paternity Leave</option>
        </select>
      </div>
      <div class="form-grid">
        <div class="form-row">
  ...
  document
    .getElementById("leave-startDate")
    .addEventListener("change", updateDuration);
  document
    .getElementById("leave-endDate")
    .addEventListener("change", updateDuration);
  ...
  document
    ...
    .addEventListener("submit", async (event) => {
      ...
      try {
        ...
        submitButton.disabled = false;
        if (error.fields) showFormErrors(error.fields, "leave-");
        toast(error.message || "Could not submit request", "error");
      }
    });
}
...
function calcDays(start, end) {
  return (
    Math.ceil(
      Math.abs(new Date(end).getTime() - new Date(start).getTime()) / 86400000,
    ) + 1
  );
}
...
function renderAttendance() {
  ...
  const avgHours =
    filtered.length > 0 ? (totalHours / filtered.length).toFixed(1) : "0";
  const weeklyData = weeklyAttendanceTrend(ui.attendanceDate);
  const statusData = [
  ...
  const stats = [
    {
      label: "PRESENT TODAY",
      value: presentCount,
      sub: `${filtered.length > 0 ? ((presentCount / filtered.length) * 100).toFixed(0) : 0}% rate`,
      iconName: "checkCircle",
      color: NEON_GREEN,
    },
    {
    ...
    <div class="page">
          ...
          <div class="filters">
            ...
            <select id="attendance-status" class="select" style="max-width:220px">
              ${[
                ...
                .map(
                  ([v, l]) =>
                    `<option value="${v}" ${ui.attendanceStatus === v ? "selected" : ""}>${l}</option>`,
                )
                .join("")}
            </select>
          </div>
      ...
      <div class="grid charts-grid">
        <section class="card">
          ...
          <div class="card-content">${barChart(
            weeklyData,
            [
              { key: "present", label: "Present", color: NEON_GREEN },
              { key: "late", label: "Late", color: NEON_GOLD },
              { key: "absent", label: "Absent", color: NEON_RED },
            ],
            "day",
          )}</div>
...
function attachAttendanceEvents() {
  ...
  document
    ...
    ?.addEventListener("change", (e) => {
      ui.attendanceDate = e.target.value;
      render();
    });
  document
    .getElementById("attendance-status")
    ?.addEventListener("change", (e) => {
      ui.attendanceStatus = e.target.value;
      render();
    });
}
...
function badge(text, color) {
  return `<span class="badge" style="background:${hexToRgba(color, 0.15)}; border:1px solid ${hexToRgba(color, 0.35)}; color:${color}">${escapeHtml(text)}</span>`;
}
...
function attendanceStatusIcon(status) {
  const map = {
    present: ["checkCircle", NEON_GREEN],
    absent: ["xCircle", NEON_RED],
    late: ["clock", NEON_GOLD],
    "half-day": ["calendar", MUTED],
  };
...
function lineChart(data, series, xKey) {
  ...
  const grid = [0, 0.25, 0.5, 0.75, 1]
    .map(
      (p) =>
        `<line class="grid-line" x1="${left}" x2="${width - right}" y1="${top + chartH * p}" y2="${top + chartH * p}"/>`,
    )
    .join("");
  const lines = series
    .map((s) => {
      ...
      const dots = data
        .map(
          (d, i) =>
            `<circle cx="${x(i)}" cy="${y(d[s.key])}" r="3" fill="${s.color}" filter="drop-shadow(0 0 4px ${s.color})"><title>${s.label}: ${d[s.key]}</title></circle>`,
        )
        .join("");
  ...
  const labels = data
    .map(
      (d, i) =>
        `<text class="axis-text" x="${x(i)}" y="${height - 12}" text-anchor="middle">${escapeHtml(d[xKey])}</text>`,
    )
    .join("");
...
function pieChart(data, nameKey, valueKey, colorMap = null) {
  ...
  const paths = data
    .map((d, i) => {
      ...
      const color =
        colorMap?.[d[nameKey]] || CHART_COLORS[i % CHART_COLORS.length];
      angle += slice;
