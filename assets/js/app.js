...
function attachPayrollEvents() {
  ...
  document.querySelectorAll("[data-download-payslip]").forEach((button) =>
    button.addEventListener("click", () => {
      const rec = state.payrollRecords.find(
        (r) => r.id === button.getAttribute("data-download-payslip"),
      );
        ...
        <div class="card-content">
          <div class="table-wrap"><table>
            ...
            <tbody>
              ${
                filtered.length === 0
                  ? `<tr><td colspan="6" class="no-records">// NO RECORDS FOR THIS DATE — TRY 2025-07-25 TO 2025-07-29</td></tr>`
                  : filtered
                      .map((rec) => {
...
function attachAttendanceEvents() {
  document
    ...
    ?.addEventListener("click", () => toast("Attendance report exported!"));
  document
